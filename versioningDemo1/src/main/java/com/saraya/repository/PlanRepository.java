...
public class PlanRepository {
	...
	public PlanDTO fetchPlanById(int planId) {
		...
		return optionalPlanDTO.orElse(plans.get(0)); // if the optional contains a value, fine. Else the first plan

														// available in the list will be set
	...
	public Map<String, Integer> fetchPlanByIdv2(int planId) {
		...
		rates.put("localRate", optionalPlanDTO.orElse(plans.get(0)).getLocalRate());

		rates.put("nationalRate", optionalPlanDTO.orElse(plans.get(0)).getNationalRate());
